...
@section('content')
        ...
        <div class="position-relative d-flex">
            ...
            <div class="d-flex position-absolute text-white top-50 end-0 px-3 translate-middle-y">
                <div>
                    ...
                    <div class="px-3 fw-semibold h6 text-end text-white">
                        <span><button class="btn btn-success rounded-5 px-5 btn-lg">Install Now</button></span>
                        <span class="rounded-5 bg-primary p-1">{{$app->app_status()}}</span>/<span class="rounded-5 bg-primary p-1">{{$app->status()}}</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container-xl">
        ...
        <div class="text-start mt-5">
            <div class="h4 fw-bold">@lang('app.comments')</div>
            @foreach($comments as $comment)
                @if ($comment->client->name)
                    <div class="fs-6 fw-semibold"> {{$comment->client->name}} <i class="bi bi-person-fill text-success"></i></div>
                @endif
                <div class="small">{{$comment->created_at}}</div>
                <div class="mb-3 mt-2 w-50">{{ $comment->comment }}</div>
            @endforeach
        </div>
