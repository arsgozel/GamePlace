...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
            <a href="{{ route('manager.types.create') }}" class="btn btn-danger btn-sm">
        ...
        <table class="table table-hover table-striped table-bordered">
            ...
            @foreach($objs->sortBy('sort_order')->sortBy('parent.sort_order') as $obj)
                <tr>
                    ...
                    <td>
                        <img src="{{  $obj->getImage() }}" alt="{{ $obj->getName() }}" class="img-fluid rounded-5" style="max-height:5rem;">
                    </td>
                    <td>
                        @if($obj->parent_id)
                            {{ $obj->parent->name_tm }} /
                        @endif
                        <a href="{{ route('manager.types.index', $obj->slug) }}" class="text-decoration-none">
                            {{ $obj->name_tm }} <i class="bi-box-arrow-up-right"></i>
                        </a>
                    </td>
                    <td>
                        @if($obj->parent_id)
                            {!! $obj->parent->name_en ?: '<span class="text-danger">' . $obj->parent->name_tm . '</span>' !!} /
                        @endif
                        {!! $obj->name_en ?: '<span class="text-danger">' . $obj->name_tm . '</span>' !!}
                    </td>
                    <td>
                        @if($obj->parent_id)
                            {!! $obj->parent->name_ru ?: '<span class="text-danger">' . $obj->parent->name_tm . '</span>' !!} /
                        @endif
                        {!! $obj->name_ru ?: '<span class="text-danger">' . $obj->name_tm . '</span>' !!}
                    </td>
                    <td>
                        <a href="{{ route('manager.types.edit', $obj->id) }}" class="btn btn-success btn-sm my-1">
                            <i class="bi-pencil"></i>
                        </a>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <form action="{{ route('manager.types.destroy', $obj->id) }}" method="post">
                                            @method('DELETE')
                                            @csrf
                                            @honeypot
